...
- なるべく遷移しないようにページ数は少なくフォームや編集などはモーダルなどを使用しスラスラと登録や編集ができる様にしました
...
- Submit ボタンが押されたら、DB 操作が終わるまで再度ボタンを押せない様にし、<br>loadingSpinner アニメーションを使用しロード中だとユーザーにお知らせできるようにしました。
