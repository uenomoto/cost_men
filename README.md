...
- Auth0 という認証サービスを採用し、セキュリティ面を強化。

- ソーシャルログイン機能を提供し、LINE ログインもサポート。
...
- なるべく遷移しないようにページ数は少なくフォームや編集などはモーダルなどを使用しスラスラと登録や編集ができるようにしました

<br>

- ユーザーが登録や編集、削除などアクションを起こすとメッセージがアナウンスされる様にし、<br>今ユーザーは何をしたのかを成功や失敗など色分け表示するようにしました
...
- バリデーションはバックエンドは勿論、レシピ登録以外はフロント側でもバリデーションがあり<br>リアルタイムで focus と value を監視し検証に引っ掛かったら教えてあげれるようにしました。

<br>

- バリデーションに引っ掛かったり、何もしていない場合は常に Submit ボタンが非活性になり DB に送信できない様にしました。<br>バリデーションがクリアできたら押せるようになります。

<br>

- Submit ボタンが押されたら、DB 操作が終わるまで再度ボタンを押せないようにし、<br>loadingSpinner アニメーションを使用しロード中だとユーザーにお知らせできるようにしました。
  <br>何度もクリックされ DB に不具合が起きないようにする対策にもなります。win-win です！

**データベース操作と API**

- Rails の form_object を使用し 3 つのテーブル(model)を同時に DB 操作できるようにして API の数を減らし、<br>ユーザーからとしても 1 ページで登録、編集更新ができるのでいいと思いました。

- 一度の API で複数の処理はトランザクションを使用し一貫性(全て完了か全て失敗)と整合性(矛盾が無い)を保ちながら DB 操作できるようにしました。

**開発環境と運用**

- CI/CD パイプラインは開発前に準備し、開発前に本番環境を整え、こまめに素早くデプロイできるようにしました。
...
次の動画で注目して欲しいのはしっかりと 1/円が変わりレシピに対する原材料の単価も変わっていることです
...
最後にログアウトをし URL からアクセスされても認可されていなければ認証必須ページには遷移できません。
